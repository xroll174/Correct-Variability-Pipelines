...
do
    ...
    FILE=data/${sub[i]}/fsl_analysis/smooth_8/reg_0/der_0.feat/reg_standard/stats/cope1.nii
    if ! [ -f "$FILE" ]; then

	rm -r data/${sub[i]}/fsl_analysis/smooth_8/reg_0/der_0.feat

	rm src/design_fsf/design_fullproc_${sub[i]}_8_0_0.fsf
	
	python3 -c "import sys; sys.path.insert(1,'src/src'); from script_generation_fullproc import script_gen_fullproc; script_gen_fullproc(${sub[i]},8,0,0,'$fulldir','$fslpath');"

	feat src/design_fsf/design_fullproc_${sub[i]}_8_0_0.fsf

	mkdir -r data/${sub[i]}/fsl_analysis/smooth_8/reg_0/der_0.feat/reg

	scp -r data/${sub[i]}/fsl_analysis/smooth_5/reg_0/der_0.feat/reg/* data/${sub[i]}/fsl_analysis/smooth_8/reg_0/der_0.feat/reg
	
	updatefeatreg data/${sub[i]}/fsl_analysis/smooth_8/reg_0/der_0.feat -gifs

	featregapply data/${sub[i]}/fsl_analysis/smooth_8/reg_0/der_0.feat

	gunzip data/${sub[i]}/fsl_analysis/smooth_8/reg_0/der_0.feat/reg_standard/stats/cope1.nii.gz
	
	octave --eval "addpath $spmpath; addpath $srcpath; masking_fsl(${sub[i]},8,0,0)"

	rm -r data/${sub[i]}/fsl_analysis/smooth_8/reg_0/der_0.feat/reg
    fi
done
