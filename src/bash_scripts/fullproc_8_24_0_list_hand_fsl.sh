...
do
    ...
    FILE=data/${sub[i]}/fsl_analysis/smooth_8/reg_24/der_0.feat/reg_standard/stats/cope1.nii
    if ! [ -f "$FILE" ]; then

	rm -r data/${sub[i]}/fsl_analysis/smooth_8/reg_24/der_0.feat

	rm src/design_fsf/design_fullproc_${sub[i]}_8_24_0.fsf
	
	python3 -c "import sys; sys.path.insert(1,'src/src'); from script_generation_fullproc import script_gen_fullproc; script_gen_fullproc(${sub[i]},8,24,0,'$fulldir','$fslpath');"

	feat src/design_fsf/design_fullproc_${sub[i]}_8_24_0.fsf

	mkdir -r data/${sub[i]}/fsl_analysis/smooth_8/reg_24/der_0.feat/reg

	scp -r data/${sub[i]}/fsl_analysis/smooth_5/reg_0/der_0.feat/reg/* data/${sub[i]}/fsl_analysis/smooth_8/reg_24/der_0.feat/reg
	
	updatefeatreg data/${sub[i]}/fsl_analysis/smooth_8/reg_24/der_0.feat -gifs
