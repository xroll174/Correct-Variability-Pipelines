...
do
    ...
    FILE=data/${sub[i]}/analysis_fsl/smooth_5/reg_24/der_0.feat/stats/cope1.nii
    if ! [ -f "$FILE" ]; then
	python3 -c "import sys; sys.path.insert(1,'src/src'); from script_generation_fla import script_gen_fla; script_gen_fla(${sub[i]},5,24,0,'$fulldir','$fslpath');"

	feat src/design_fsf/design_fla_${sub[i]}_5_24_0.fsf

	mkdir -r data/${sub[i]}/fsl_analysis/smooth_5/reg_24/der_0.feat/reg

	scp -r data/${sub[i]}/unprocessed/3T/tfMRI_MOTOR_LR_FSL_5.feat/reg data/${sub[i]}/fsl_analysis/smooth_5/reg_24/der_0.feat/reg
