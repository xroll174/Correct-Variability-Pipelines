function [] = preprocessing_matlab(subject,smooth_value)
    ...
    if not(exist(fullfile('data',subject,'unprocessed','3T','tfMRI_MOTOR_LR',['wr',subject,'_3T_tfMRI_MOTOR_LR.nii'])))
        ...
        matlabbatch{1}.spm.spatial.normalise.write.subj.resample = cellstr([string(spm_file(f1,'prefix','r'));string(spm_file(f,'prefix','mean'))]);
    ...
    if smooth_value == '8'
        if not(exist(fullfile('data',subject,'unprocessed','3T','tfMRI_MOTOR_LR',['s8wr',subject,'_3T_tfMRI_MOTOR_LR.nii'])))            
            matlabbatch{1}.spm.spatial.smooth.data = cellstr([string(spm_file(f1,'prefix','wr'))]);
